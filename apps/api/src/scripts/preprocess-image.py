...
def preprocess_image(input_path, output_path):
    ...
    if screenCnt is None:
        # Fallback to original image if no contour found
        print("No document contour found, using original image", file=sys.stderr)
        warped = orig
    else:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python preprocess-image.py <input> <output>", file=sys.stderr)
        sys.exit(1)
