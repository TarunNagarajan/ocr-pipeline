...
export default function DocumentDetailPage() {
  ...
  const [hoveredFieldData, setHoveredFieldData] = useState<{evidenceIds: string[], spatialBboxes?: number[][]}>({evidenceIds: []});
        ...
        <div className="flex h-full flex-col">
          <div className="flex h-[42px] items-center justify-between border-b border-[var(--border-subtle)] bg-[var(--surface-toolbar)] px-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="rounded-md bg-black/20 px-3 py-1 text-sm text-[var(--text-primary)]">{zoom}%</div>
                ...
                <div style={{ transform: `scale(${zoom / 180})`, transformOrigin: "top left" }}>
                  {payload.mimeType === "application/pdf" ? (
                    <div className="relative inline-block bg-white shadow-2xl">
                      ...
                      {showHighlights && (
                        <BoundingBoxOverlay evidenceIndex={evidenceIndex} hoveredData={hoveredFieldData} />
                      )}
                    </div>
                  ) : (
                    <div className="relative inline-block bg-white shadow-2xl">
                      ...
                      />
                      {showHighlights && typeof (window as any) !== 'undefined' && (hoveredFieldData.evidenceIds.length > 0 || hoveredFieldData.spatialBboxes) && (
                         // Simple visual highlighter for images if needed, or omit if iframe is only for PDFs.
                         // The user asked to revert to the firefox one and do highlighting on it. We did that for PDFs.
                         <BoundingBoxOverlay evidenceIndex={evidenceIndex} hoveredData={hoveredFieldData} />
                      )}
                    </div>
                  )}
...
function BoundingBoxOverlay({ evidenceIndex, hoveredData }: { evidenceIndex: Map<string, EvidenceBlock>, hoveredData: {evidenceIds: string[], spatialBboxes?: number[][]} }) {
  if (hoveredData.evidenceIds.length === 0 && !hoveredData.spatialBboxes) return null;

  return (
    <div className="absolute inset-0 pointer-events-none z-50">
      {hoveredData.spatialBboxes ? (
        hoveredData.spatialBboxes.map((bbox, idx) => {
          ...
          return (
            <div
              key={`spatial-${idx}`}
              className="absolute border-2 border-yellow-400 bg-yellow-400/30 transition-all shadow-[0_0_8px_rgba(250,204,21,0.6)] rounded-sm"
              style={{ left, top, width, height }}
            />
          );
        })
      ) : (
        hoveredData.evidenceIds.map(id => {
          const block = evidenceIndex.get(id);
...
function StructuredPane(props: {
  ...
  onHoverField?: (hoverData: {evidenceIds: string[], spatialBboxes?: number[][]}) => void;
}) {
          ...
          <div className="grid gap-3">
            {group.fields.map(([label, field]) => (
              <ExtractionFieldCard 
                ...
                label={label} 
                field={field} 
                onHover={(isHovering) => props.onHoverField?.(isHovering ? {evidenceIds: field.evidenceIds, spatialBboxes: field.spatialBboxes} : {evidenceIds: []})}
              />
            ))}
          </div>
        </section>
